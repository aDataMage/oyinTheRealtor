...
const MobileDialog = ({ links, setIsMobileDialogOpen }) => {
    ...
    if (!isPresent) {
      ...
      const enterAnimation = async () => {
        ...
        await animate(
          "li",
          { opacity: 1, y: 0 },
          { duration: 0.4, ease: "easeIn", delay: stagger(0.1) }
        );
      };
      enterAnimation();
    }
  ...
  return (
    ...
    >
      <motion.div
        layoutId="logo"
        className="flex absolute top-4 left-4 items-center"
      >
      ...
      <motion.ul className="flex flex-col mb-10 space-y-4 text-white text-2xl">
        {links.map((link) => (
          <motion.li
